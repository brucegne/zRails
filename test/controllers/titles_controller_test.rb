...
class TitlesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create title" do
    assert_difference('Title.count') do
      post titles_url, params: { title: { name: @title.name, notes: @title.notes, string: @title.string, text: @title.text } }
    end
  ...
  test "should update title" do
    patch title_url(@title), params: { title: { name: @title.name, notes: @title.notes, string: @title.string, text: @title.text } }
